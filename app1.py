...
def get_vector_store_and_llm(documents):
    if 'vectordb' not in st.session_state:
        if os.path.exists(VECTOR_STORE_PATH):
            vectordb = FAISS.load_local(VECTOR_STORE_PATH, OllamaEmbeddings(), allow_dangerous_deserialization=True)
        else:
...
# Streamlit App
def main():
    st.set_page_config("Sales Pitch Bot")
    ...
    if st.button("Generate"):
        if input_query:  # Check if the query is not empty
            st.session_state.chat_history.append(f"Q: {input_query}")
            prompt = ChatPromptTemplate.from_template("""
Your role is of a successful and experienced Sales Manager currently working for a leading computer hardware manufacturer. 
...
5. Based on the extracted data in the previous step, provide a customer profile from the background section of the case studies and competitor profile from the competition section of the case studies, before the actual sales pitch
...
7. If the question requires information about the customer or the competitor profiles, then just provide the extracted customer or competitor profile from previous steps, and do not generate the sales pitch in this case. 

8. When you genrate the customer or competitor profile for step 7, DO NOT MAKE UP ANY DATA for your generated response.

9. If you are unable to get the context or identify that it is not a Sales related query, do not make up an answer, reply with the following statement "Unable to generate pitch"
<context>
{context}
</context>
Question: {input}
""")
            ...
            st.session_state.chat_history.insert(0, formatted_response)  # Insert response at the beginning of the list
            for message in st.session_state.chat_history:
                st.markdown(message, unsafe_allow_html=True)  # Using Markdown to format the output
        else:
